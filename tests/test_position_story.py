...
def test_roll_up_and_out_detected():
    ...
    items, markers = build_position_story(df, None)
...
def test_wheel_full_cycle_narrated():
    ...
    items, _ = build_position_story(df, None)
    text = _headlines(items)
...
def test_dividends_and_marker_alignment():
    ...
    items, markers = build_position_story(df, div)
    text = _headlines(items)
...
def test_interlude_from_daily_marks():
    ...
    items, _ = build_position_story(df, None, chart)
...
def test_split_is_narrated_and_fixes_share_units():
    ...
    items, markers = build_position_story(df, None, splits_df=splits)
    text = _headlines(items)
...
def test_split_while_flat_is_silent():
    ...
    items, _ = build_position_story(df, None, splits_df=splits)
