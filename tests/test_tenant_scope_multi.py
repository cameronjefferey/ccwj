...
def test_tenants_param_all_unowned_shows_nothing():
    """An explicit tenant the user does not own must not become All."""
    scope = _resolve("?tenants=snaptrade:HACK1,snaptrade:HACK2")
    assert scope == []


def test_unknown_tenant_param_shows_nothing():
    scope = _resolve("?tenant=snaptrade:nope")
    assert scope == []


def test_unknown_tenant_flashes_once_per_request():
    from app import app
    import app.routes as routes
    from flask import get_flashed_messages

    user = SimpleNamespace(id=9, username="cam", is_authenticated=True)
    with app.test_request_context("/accounts?tenant=snaptrade:nope"):
        with patch.object(routes, "current_user", user), \
    ...
    assert "isn't on your profile" in messages[0][1]
