...
select
    ...
    round(100.0 * sum(case when a.outcome = 'Winner' then a.num_trades else 0 end)
          over (partition by a.account, a.strategy, a.dte_bucket, a.moneyness_at_open)
          / nullif(sum(a.num_trades) over (partition by a.account, a.strategy, a.dte_bucket, a.moneyness_at_open), 0), 1) as win_rate_pct,
...
order by a.account, st.strategy_total_pnl desc nulls last, a.strategy, a.dte_bucket, a.moneyness_at_open, a.outcome
