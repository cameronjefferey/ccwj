...
select
    ...
    round(100.0 * sum(case when a.outcome = 'Winner' then a.num_trades else 0 end)
          over (partition by a.tenant_id, a.account, a.user_id, a.strategy, a.dte_bucket, a.moneyness_at_open)
          / nullif(sum(a.num_trades) over (partition by a.tenant_id, a.account, a.user_id, a.strategy, a.dte_bucket, a.moneyness_at_open), 0), 1) as win_rate_pct,
...
join strategy_totals st
    ...
    and (a.tenant_id is not distinct from st.tenant_id)
    and a.strategy = st.strategy
order by a.tenant_id, a.account, a.user_id, st.strategy_total_pnl desc nulls last, a.strategy, a.dte_bucket, a.moneyness_at_open, a.outcome
