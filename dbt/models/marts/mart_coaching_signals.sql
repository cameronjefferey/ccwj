...
base as (
    select distinct account, strategy
    from {{ ref('int_option_exit_analysis') }}
)
...
left join exit_stats e
    on b.account = e.account and b.strategy = e.strategy
left join roll_stats r
    on b.account = r.account
left join best_dte bd
    on b.account = bd.account and b.strategy = bd.strategy
left join worst_dte wd
    on b.account = wd.account and b.strategy = wd.strategy
