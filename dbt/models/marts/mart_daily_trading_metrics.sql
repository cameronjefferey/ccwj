...
symbol_volumes as (
    select account, trade_date, symbol, sum(position_size) as vol
    from trades
    group by 1, 2, 3
),
...
strategies_on_date as (
    select distinct t.account, t.trade_date, s.strategy
    from trades t
...
left join concentration c
    on d.account = c.account and d.trade_date = c.trade_date
left join strategy_agg sa
    on d.account = sa.account and d.trade_date = sa.trade_date
left join holding_times ht
    on d.account = ht.account and d.trade_date = ht.trade_date
order by d.account, d.trade_date
