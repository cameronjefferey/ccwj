...
the nav says so in language that matches pre-market vs the open vs after the bell.
...
- Session trades: every fill dated the **review session** from `stg_history`
 ...
 session is withheld, the strip under the nav says so in language that
 matches the session: pre-market names last close and the 9:30 ET open;
 the regular session says today is live and Overview still shows last
 close; after the bell, today's close lands tomorrow. Adds and
...
- Since you last looked: stock moves / newly ITM / newly near expiry /
  opens & closes vs the previous Today visit (`bump_review_visit` stamps
  here, not Overview — home redirects to Overview and would zero the
  gap). Still shown on pre-market / weekend Today. 24h / calendar-day
  gate; `?from_sync=1` / `?from_upload=1` force-shows.
...
- **PWA install** (`app/static/manifest.webmanifest`, `app/static/sw.js`
  ...
  cached (auth pages must not be servable stale). Account → Install app
  is a button in the menu (not `href="#"`). nav.js calls Chromium's
  `beforeinstallprompt` when the browser has one; otherwise it opens
  `#htInstallModal` with Safari / home-screen steps. The item hides when
